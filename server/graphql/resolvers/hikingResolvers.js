...
const trailSample = require('../../data/hikeSample.json');

//Trail API with optional filters as defined in the API documentation. Only utilized once a value is set.
module.exports = {
  Query: {
    getTrails: async (_root, { lat, lon, maxDist, maxResults, minLen, minStars }, request) => {
      ...
      // return await axios
      //   .get(
      //     `${apiRoute}?lat=${lat}&lon=${lon}${maxDist ? `&maxDistance=${maxDist}` : ''}${
      //       maxResults ? `&maxResults=${maxResults}` : '&maxResults=35'
      //     }${minLen ? `&minLength=${minLen}` : ''}${minStars ? `&minStars=${minStars}` : ''}&key=${key}`
      //   )
      //   .then((resp) => {
